...
//    }
    ...
    public static User[] loadAllByGroupId(Connection conn, int groupId) throws SQLException {
        ...
        ResultSet usersOfGroupSet = prepStm.executeQuery();
        while (usersOfGroupSet.next()) {
            User loadedUser = new User();
            loadedUser.id = usersOfGroupSet.getInt("id");
            loadedUser.userName = usersOfGroupSet.getString("user_name");
            loadedUser.password = usersOfGroupSet.getString("password");
            loadedUser.email = usersOfGroupSet.getString("email");
            usersOfGroup.add(loadedUser);
        }
        User[] uArray = new User[usersOfGroup.size()];
        uArray = usersOfGroup.toArray(uArray);
        return uArray;
    }
